...
//			{ "url": "", "zhflag": "none, trad, simp" }
	...
	safari.self.tab.dispatchMessage("OptionsSave", null);
...
function btnLoginLogout() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			for (var i in btns) {
				if (["登出", "登入"].indexOf(i) >= 0) continue;
				nbtns[i] = btns[i];
			}
			$("#msgInOutDialog").dialog("option", "buttons", nbtns);
		}
	});
}
